  ...
  Knob = function (inputEl, callback) {
    ...
    */
    ...
    if (inputEl.hasAttribute("data-angle-value-ratio")) {
      options.angleValueRatio = parseFloat(
        inputEl.getAttribute("data-angle-value-ratio")
      );
    }
    if (inputEl.hasAttribute("data-angle-slide-ratio")) {
      options.angleSlideRatio = parseFloat(
        inputEl.getAttribute("data-angle-slide-ratio")
      );
    }
    if (inputEl.hasAttribute("data-angle-scroll-ratio")) {
      options.angleScrollRatio = parseFloat(
        inputEl.getAttribute("data-angle-scroll-ratio")
      );
    }
    if (inputEl.hasAttribute("data-gesture-spin-enabled")) {
      options.gestureSpinEnabled = parseBool(
        inputEl.getAttribute("data-gesture-spin-enabled")
      );
    }
    if (inputEl.hasAttribute("data-gesture-slidex-enabled")) {
      options.gestureSlideXEnabled = parseBool(
        inputEl.getAttribute("data-gesture-slidex-enabled")
      );
    }
    if (inputEl.hasAttribute("data-gesture-slidey-enabled")) {
      options.gestureSlideYEnabled = parseBool(
        inputEl.getAttribute("data-gesture-slidey-enabled")
      );
    }
    if (inputEl.hasAttribute("data-gesture-scroll-enabled")) {
      options.gestureScrollEnabled = parseBool(
        inputEl.getAttribute("data-gesture-scroll-enabled")
      );
    }
    if (inputEl.hasAttribute("data-center-offset-x")) {
      options.centerOffsetX = parseFloat(
        inputEl.getAttribute("data-center-offset-x")
      );
    }
    if (inputEl.hasAttribute("data-center-offset-y")) {
      options.centerOffsetY = parseFloat(
        inputEl.getAttribute("data-center-offset-y")
      );
    }
    if (inputEl.hasAttribute("data-indicator-auto-rotate")) {
      options.indicatorAutoRotate = parseBool(
        inputEl.getAttribute("data-indicator-auto-rotate")
      );
    }
    ...
    if (inputEl.hasAttribute("data-indicator-start-angle")) {
      options.indicatorStartAngle = parseFloat(
        inputEl.getAttribute("data-indicator-start-angle")
      );
    }
    if (inputEl.hasAttribute("data-indicator-radius")) {
      options.indicatorRadius = parseFloat(
        inputEl.getAttribute("data-indicator-radius")
      );
    }
    if (inputEl.hasAttribute("data-sprite-width")) {
      options.spriteWidth = parseFloat(
        inputEl.getAttribute("data-sprite-width")
      );
    }
    if (inputEl.hasAttribute("data-sprite-height")) {
      options.spriteHeight = parseFloat(
        inputEl.getAttribute("data-sprite-height")
      );
    }
    if (inputEl.hasAttribute("data-sprite-count")) {
      options.spriteCount = parseInt(
        inputEl.getAttribute("data-sprite-count"),
        10
      );
    }
    if (inputEl.hasAttribute("data-sprite-direction")) {
      options.spriteDirection = parseDirection(
        inputEl.getAttribute("data-sprite-direction")
      );
    }
    if (inputEl.hasAttribute("data-sprite-start-angle")) {
      options.spriteStartAngle = parseFloat(
        inputEl.getAttribute("data-sprite-start-angle")
      );
    }
    ...
    if (inputEl.hasAttribute("data-sprite-separation-gap")) {
      options.spriteSeparationGap = parseFloat(
        inputEl.getAttribute("data-sprite-separation-gap")
      );
    }
  ...
  const members = {
    ...
    */
    ...
    doTouchStart: function (touches, timeStamp) {
      ...
      self.__initialAngleDiff = angleDistance(
        self.__initialAngle,
        self.__angle
      );
      ...
      self.__slideXDetected =
        !isSingleTouch && self.options.gestureSlideXEnabled;
      self.__slideYDetected =
        !isSingleTouch && self.options.gestureSlideYEnabled;

      // Figure out where the touch was relative to the center
      self.__initialTouchLocationX =
        currentTouchLeft >= self.__centerPageX ? "right" : "left";
      self.__initialTouchLocationY =
        currentTouchTop >= self.__centerPageY ? "bottom" : "top";
    ...
    doTouchMove: function (touches, timeStamp, scale) {
      ...
      self.__validateAndPublishAngle(
        self.__getAngleFromGesture(currentTouchLeft, currentTouchTop)
      );
    ...
    */
    ...
    __validateAndPublishAngle: function (angle, forcePublish) {
      ...
      self.__value = self.__valueFromAngles(
        forcePublish ? nextAngle : prevAngle,
        nextAngle
      );
    ...
    __validateAngle: function (angle, force) {
      ...
      if (!force) {
        ...
        const threshold = 30;
        if (
          self.__angle == self.options.angleStart &&
          Math.abs(angle - self.__angle) > threshold
        ) {
          angle = self.options.angleStart;
        }
        if (
          self.__angle == self.options.angleEnd &&
          Math.abs(angle - self.__angle) > threshold
        ) {
    ...
    __valueFromAngles: function (prevAngle, nextAngle) {
      ...
      const value =
        self.__value + (prevAngle - nextAngle) * self.options.angleValueRatio;
    ...
    __angleFromValue: function (value) {
      ...
      const angle =
        self.__angle + (value - self.__value) / self.options.angleValueRatio;
    ...
    __getIndicator: function (angle) {
      ...
      if (self.options.indicatorAutoPosition) {
        ...
        (indicator.x =
          self.__centerPageX -
          self.__clientLeft +
          self.options.indicatorRadius * Math.cos(rads)),
          (indicator.y =
            self.__centerPageY -
            self.__clientTop -
            self.options.indicatorRadius * Math.sin(rads));
      } else {
    ...
    __getSpriteOffset: function (angle) {
      ...
      if (self.options.spriteCount > 1) {
        let spriteAngle =
          self.options.spriteDirection === "ccw" ? angle : -angle;
        // Align the background image for sprites
        spriteAngle += self.options.spriteStartAngle;
        let imageIndex =
          Math.floor(spriteAngle / self.options.spriteSeparationAngle) %
          self.options.spriteCount;
        if (imageIndex < 0) {
    ...
    __getAngleFromGesture: function (currentTouchLeft, currentTouchTop) {
      ...
      if (self.__spinDetected) {
        ...
        if (self.__slideXDetected) {
          const change =
            (currentTouchLeft - self.__lastTouchLeft) *
            self.options.angleSlideRatio;
          angle += self.__initialTouchLocationY === "top" ? -change : change;
        }

        if (self.__slideYDetected) {
          const change =
            (currentTouchTop - self.__lastTouchTop) *
            self.options.angleSlideRatio;
    ...
    __updateCenterLocation: function () {
      ...
      self.__centerPageX =
        self.__clientLeft + self.__clientWidth / 2 + self.options.centerOffsetX;
      self.__centerPageY =
        self.__clientTop + self.__clientHeight / 2 + self.options.centerOffsetY;
    },
  };
